...
#manual override for bad BC for site  ENT00001-3A6
if (site.list$SiteID[k]== "ENT00001-3A6") {outflow.y.idx[2]=435}
...
################################################################
### Make plot porous plate locations #####
################################################################
plot(data$X, data$Y, col="gray", pch=19, cex=.1,
 ylim=c(min(data$Y), min(data$Y)+1.2*(max(data$Y)-min(data$Y))),
 xlab="Easting (m)", ylab="Northing (m)",
...
points(GridX[inlet.x, inlet.y], GridY[inlet.x, inlet.y],pch=19, cex=.7,col="blue")
lines(GridX[outflow.x.idx, outflow.y.idx], GridY[outflow.x.idx, outflow.y.idx],col="red", lw=4)


#col.idx =1+(round(9*(data[,3]-min(data[,3]))/(max(data[,3])-min(data[,3]))))
#plot(data$X, data$Y,col=rainbow(10)[col.idx],
#xlab="Easting", ylab="Northing")
points(pp.x, pp.y, col="brown")
dev.off()
...
# now we've got x and y points and a porosity value
# Write the ppl file
cat("", file = "test.ppl")
for (pp.idx in 1:length(pp.m)){
cat(file="test.ppl", append=T,paste("U", pp.m[pp.idx], pp.n[pp.idx],pp.m[pp.idx], pp.n[pp.idx], "1 1", pp.friction[pp.idx]),"\n")
cat(file="test.ppl", append=T,paste("V", pp.m[pp.idx], pp.n[pp.idx],pp.m[pp.idx], pp.n[pp.idx], "1 1", pp.friction[pp.idx]),"\n")
}

} # end of if "Jam_Locations.csv" in dir.....
...
# Remove any duplicates due to over-lapping porous structures
length(pp_Xidx)
temp=paste(pp_Xidx, pp_Yidx)
temp2=levels(factor(temp))
temp.idx = match(temp2,temp)
length(temp.idx)
pp_Xidx = pp_Xidx[temp.idx]
pp_Yidx = pp_Yidx[temp.idx]
