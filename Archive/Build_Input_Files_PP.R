...
#os= read.csv("offset.csv",header=T)
...
pps
xx= c(pp$START_X[pps]-min(GridX),pp$END_X[pps]-min(GridX)) 
yy= c(pp$START_Y[pps]-min(GridY),pp$END_Y[pps]-min(GridY)) 
xx
yy
...
dist = function(x0, x1, x2, y0, y1, y2) {
xs = seq(x1, x2, by=(x2-x1)/100)
ys = seq(y1, y2, by =(y2-y1)/100)
data.frame(xs, ys)
...
x2 = pp$END_X[pps]-min(GridX)
y1 = pp$START_Y[pps]-min(GridY)
y2 = pp$END_Y[pps]-min(GridY)
distance = X*0 + 9999
...
for (idx in 1:length(pp_Xidx)){
 points(X[pp_Xidx[idx],pp_Yidx[idx]]
,Y[pp_Xidx[idx], pp_Yidx[idx]], pch=19, cex=.1)
}
...
# Junk to Delete!!!!!
if (1==2) {
data.frame(Xidx, Yidx)
for (i in 1:length(Xidx)){
#points(X[Xidx[i],Yidx[i]], Y[Xidx[i]Yidx[i]], col="blue")
}
...
Xppl = Xidx[distance < .3]
Yppl = Xidx[distance < .3]

data.frame(Xppl, Yppl)

points(X,Y)
points(X[XYidx], Y[XYidx])


col.idx = 1+round(9*distance/max(distance))
plot(X,Y, col=rainbow(10)[col.idx])
...
###################################################
# cycle through all rows, manually
pp.idx = 1
pp$START_X
M_pp_start=(1:M)[abs(X[,1]-(pp$START_X[pp.idx]-min(GridX)))==min(abs(X[,1]-(pp$START_X[pp.idx]-min(GridX))))]
M_pp_mid=(1:M)[abs(X[,1]-(pp$MID_X[pp.idx]-min(GridX)))==min(abs(X[,1]-(pp$MID_X[pp.idx]-min(GridX))))]
M_pp_end=(1:M)[abs(X[,1]-(pp$END_X[pp.idx]-min(GridX)))==min(abs(X[,1]-(pp$END_X[pp.idx]-min(GridX))))]

N_pp_start=(1:M)[abs(Y[1,]-(pp$START_Y[pp.idx]-min(GridY)))==min(abs(Y[1,]-(pp$START_Y[pp.idx]-min(GridY))))]
N_pp_mid=(1:M)[abs(Y[1,]-(pp$MID_Y[pp.idx]-min(GridY)))==min(abs(Y[1,]-(pp$MID_Y[pp.idx]-min(GridY))))]
N_pp_end=(1:M)[abs(Y[1,]-(pp$END_Y[pp.idx]-min(GridY)))==min(abs(Y[1,]-(pp$END_Y[pp.idx]-min(GridY))))]

# Let's see where we're at in MN space
M_pp_start
M_pp_mid
N_pp_start
N_pp_mid
...
plot(data$X-min(GridX), data$Y-min(GridY),col=rainbow(10)[col.idx])
points(X[Xidx,Yidx],Y[Xidx,Yidx],cex=.1)

#############################################
cat("", file = "test.ppl")
length(Ms)
for (pp.idx in 1:length(Xidx)){
cat(file="test.ppl", append=T,paste("U", Xidx[pp.idx], Yidx[pp.idx],Xidx[pp.idx], Yidx[pp.idx], "1 1 25"),"\n")
cat(file="test.ppl", append=T,paste("V", Xidx[pp.idx], Yidx[pp.idx],Xidx[pp.idx], Yidx[pp.idx], "1 1 25"),"\n")
}
...
# Add porous structure lines to plot
lines(
x=c(X[M_pp_start, N_pp_start],X[M_pp_mid, N_pp_mid]),
y=c(Y[M_pp_start, N_pp_start],Y[M_pp_mid, N_pp_mid]),lw=3
)
lines(
x=c(X[M_pp_mid, N_pp_mid],X[M_pp_end, N_pp_end]),
y=c(Y[M_pp_mid, N_pp_mid],Y[M_pp_end, N_pp_end]),lw=3
)


# Now just plot porous structure in MN space
plot(x=c(M_pp_start, M_pp_end), y=c(N_pp_start, N_pp_end), type="l")
lines(x=c(M_pp_start, M_pp_mid), y=c(N_pp_start, N_pp_mid), col="blue")
lines(x=c(M_pp_mid, M_pp_end), y=c(N_pp_mid, N_pp_end), col="blue")
...
# Manual process to create approximate porous structures 
# need to get from start-end through mid w/ only mults of 45 degree angles.  Yuck.
Ms1 = c(627,630, 631,634, 634,636, 637,641,  641,643, 644,648, 648,650, 651,654)
Ns1 = c(310,313, 313,313, 314,316, 316,316,  317,319, 319,319, 320,322, 322,322)

Ms2 = c(576, 580,    580, 580,   581, 583,   583, 583,   584, 587,   587, 587)
Ns2 = c(526, 522,    521, 515,   514, 512,   511, 506,   505, 502,   501, 497)

Ms3 = c(231, 231, 231, 232, 232, 232, 232, 233, 233, 233)
Ns3 = c(579, 560, 559, 558, 557, 540, 539, 538, 537, 530)


# Combined all into a single dataframe.
Ms= c(Ms1, Ms2, Ms3)
Ns = c(Ns1, Ns2, Ns3)
#########################################
...
cat("", file = "test.ppl")
length(Ms)
for (pp.idx in 1:(length(Ms)/2)){
lines(c(Ms[1+2*(pp.idx-1)], Ms[2*(pp.idx)]),c(Ns[1+2*(pp.idx-1)], Ns[2*(pp.idx)]),col="red")

cat(file="test.ppl", append=T,paste("U", Ms[1+2*(pp.idx-1)], Ns[1+2*(pp.idx-1)], Ms[2*pp.idx], Ns[2*pp.idx], "1 1 25"),"\n")
cat(file="test.ppl", append=T,paste("V", Ms[1+2*(pp.idx-1)], Ns[1+2*(pp.idx-1)], Ms[2*pp.idx], Ns[2*pp.idx], "1 1 25"),"\n")

}
} # End of Junk to delete (if it works!!!!)
